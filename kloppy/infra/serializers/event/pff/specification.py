...
class PASS(POSSESSION_EVENT):
    ...
    def outcome(self) -> Union[OUTCOME, CROSS_OUTCOME, None]:
        try:
            return self.OUTCOME(
                self.possession_event["passOutcomeType"]
            ) or self.CROSS_OUTCOME(self.possession_event["crossOutcomeType"])
        except Exception:
            return None
...
class DUEL(POSSESSION_EVENT):
    ...
    def duel_type(self):
        try:
            return self.TYPE(self.raw_event["possessionEvents"]["challengeType"])
        except Exception:
            return None
    ...
    def _handle_dribble(
        ...
        if self.outcome in [DUEL.OUTCOME.OUT_OF_PLAY, DUEL.OUTCOME.FORCED_OUT_OF_PLAY]:
    ...
    def _handle_aerial(
        ...
        winner = find_player(
            self.raw_event["possessionEvents"]["challengeWinnerPlayerId"], self.teams
        )
        ...
        if winner is None:
            ...
            player_duel_result = [
                (
                    player,
                    (DuelResult.WON if player.team == winner.team else DuelResult.LOST),
                )
                for player in players_involved
                if player is not None
            ]
    ...
    def _handle_gk(
        ...
        qualifiers = [GoalkeeperQualifier(value=GoalkeeperActionType.SMOTHER)]
